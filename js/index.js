...
function setupProjectsSection() {
  ...
  ; (function () {
    ...
    if (frame) {
      ...
      if (frameImg) tl.to(frameImg, { yPercent: 30, duration: frameDur, ease: 'none' }, pairStart);
    }
    ...
    if (frame2) {
      ...
      if (frameImg2) tl.to(frameImg2, { yPercent: 30, duration: frameDur, ease: 'none' }, pairStart + 0.07);
    }

    if (dispo2) {
      gsap.set(dispo2, { yPercent: -50, y: function () { return window.innerHeight * 1.35; }, opacity: 1, clipPath: 'inset(0% 0 0% 0)' });
      tl.to(dispo2, { y: frameYEnd, duration: frameDur, ease: 'none' }, pairStart + 0.08);
      tl.to(dispo2, { opacity: 0, clipPath: 'inset(100% 0 0% 0)', duration: 0.15, ease: 'power2.in' }, pairStart + 0.53);
    }
  })();
  ...
  ; (function () {
    ...
    loadAndRender('assets/images/footer/left.png', 'ascii-center', 85);

    var asciiCenterWrap = document.querySelector('.footer-ascii.center');
    if (asciiCenterWrap) {
      gsap.fromTo(asciiCenterWrap,
        { scale: 0.7, opacity: 0 },
        {
    ...
    var asciiCenterPre = document.getElementById('ascii-center');
    ...
    function parallaxLoop() {
      ...
      var px = sx * 25;
      var py = sy * 15;
      if (asciiCenterPre) asciiCenterPre.style.transform = 'translate(' + px + 'px, ' + py + 'px)';
  ...
  const PROJECTS = {
    'void': {
      desc: "A smart Chrome extension that dynamically organizes tabs, reduces cognitive noise, and manages active browser workflows through automated AI classification.",
      category: 'AI Workspace', year: '2026', tags: ['Chrome Extension API', 'TensorFlow.js', 'React'],
      link: 'https://github.com/flynnmaxweld/void-ai-workspace-manager.git',
      images: ['assets/images/projects/Covers/1.jpg'],
    },
    'aris': {
      desc: "A security framework that uses large language models to simulate and stress-test system-level vulnerabilities — turning AI into a defensive tool.",
      category: 'Tactical Intelligence', year: '2026', tags: ['Python', 'OpenAI API', 'HuggingFace'],
      images: ['assets/images/projects/Covers/2.jpg'],
    },
    'smart-file-organizer': {
      desc: "A headless Python daemon that silently classifies, labels, and routes digital assets in the background using content analysis and metadata extraction.",
      category: 'Systems Automation', year: '2026', tags: ['Python', 'Watchdog Daemon', 'PyExifTool'],
      link: 'https://github.com/flynnmaxweld/smart-file-organizer.git',
      images: ['assets/images/projects/Covers/3.jpg'],
    },
  };
  ...
  function openProject(id, clickedItem) {
    ...
    var tagsHTML = proj.tags.map(function (t) { return '<span class="detail-tag">' + t + '</span>'; }).join('');
    if (proj.link) {
      tagsHTML += '<a class="detail-tag detail-github-btn" href="' + proj.link + '" target="_blank" rel="noopener noreferrer" style="background:#ff3b14;color:#ffffff;font-weight:bold;text-decoration:none;display:inline-flex;align-items:center;gap:0.3rem;">GitHub Repo 🡺</a>';
    }
